...
struct SelectPayload_t
{
    std::string tag;
    int x = 0;
    int y = 0;
    SelectPayload_t(std::string tag, int x, int y)
    {
        this->tag = tag;
        this->x = x;
        this->y = y;
...
std::string AppDuckling::_select_payload()
{
    int8_t selected = 0;
    bool is_select_change = true;
    std::vector<SelectPayload_t> item_list;

    int entryCounter = 1;
    for (std::string const &i : _data.payload_file_list)
    {
        // Place item on an interval of "17"
        item_list.push_back(SelectPayload_t(i, 0, (17 * entryCounter)));
    ...
    while (showSelection)
    {
        ...
        if (is_select_change)
        {
            ...
            _data.hal->canvas()->print("[Select Payload]");
    ...
    if (showSelection)
    {
        return item_list[selected].tag;
    }

    // Return empty if nothing was selected
    return "";
}
